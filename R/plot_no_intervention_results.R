...
sim_results <- fread(dir_csv, header = T) %>% 
  ...
  group_by(dates,Type) %>% 
  summarise(means=mean(counts),Low=quantile(counts,probs = 0.1),High=quantile(counts,probs = 0.9))  

# make plot
ggplot(sim_results, aes(as.Date(dates), y = means/n_pop,color=Type) ) +
  geom_ribbon( aes(ymin = Low/n_pop, ymax = High/n_pop, fill = Type, color = NULL), alpha = .15) +
  geom_line( aes(y = means/n_pop), size = 1)+ 
  xlab("") +ylab("Percent")+ggtitle("Cumulative percentages of model population")+
