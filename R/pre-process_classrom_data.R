...
##input_path = "/rproject/bucovid/katia/BU-COVID/Data/input/"
##output_path = "./"
...
getClassNet <- function( k_reg, AffiMat)
{
  ...
  for (i in 1:NumofClass) {
    ...
    {
      nonzero_entry <- which(AffiMat[,i]>0 )
      ClassNet[[i]][nonzero_entry,nonzero_entry] <- get.adjacency(sample_k_regular(ClassSize[i],k_reg))
    }
  }
  return(ClassNet)
}
...
getDailyNetSparse <- function(k_reg, AffiMat, ClassSch, instructorList)
{
  ...
  ClassSize <- colSums(AffiMat)
  for (i in 1:ncol(AffiMat)) {
    instructorList_ind <- which(instructorList[,1]==NameOfClass[i])
    classDay <- sapply(lapply(ClassSch, function(ch) grep(NameOfClass[i], ch)), function(x) length(x) > 0) 
    ...
    if(sum(classDay) ==1)
    {
      if(ClassSize[i]<=k_reg & ClassSize[i]>1)
      {
        DailyNetSparse[[ seq_len(7)[classDay] ]][nonzero_entry,nonzero_entry] <-  DailyNetSparse[[ seq_len(7)[classDay] ]][nonzero_entry,nonzero_entry]+ 1 - diag(length(nonzero_entry))
      }
      if(ClassSize[i]>k_reg)
      {
        DailyNetSparse[[ seq_len(7)[classDay] ]][nonzero_entry,nonzero_entry] <-  DailyNetSparse[[ seq_len(7)[classDay] ]][nonzero_entry,nonzero_entry]+ get.adjacency(sample_k_regular(ClassSize[i],k_reg))
      }
    }
    # case 2
    if(NumOfStudent==1 & sum(classDay) >1 )
    {
      day_ind <- seq_len(7)[classDay][sample(sum(classDay),1)]
      if(ClassSize[i]<=k_reg & ClassSize[i]>1)
      {
        DailyNetSparse[[ day_ind ]][nonzero_entry,nonzero_entry] <-  DailyNetSparse[[ day_ind ]][nonzero_entry,nonzero_entry]+ 1 - diag(length(nonzero_entry))
      }
      if(ClassSize[i]>k_reg)
      {
        DailyNetSparse[[ day_ind ]][nonzero_entry,nonzero_entry] <-  DailyNetSparse[[ day_ind ]][nonzero_entry,nonzero_entry]+ get.adjacency(sample_k_regular(ClassSize[i],k_reg))
      }
    }
    # case 3
    if(NumOfStudent>1 & sum(classDay) >1)
    {
      ...
      if(length(instructorList_ind)>=1)
      {
        instructor_id <- instructorList[instructorList_ind,2]
        nonzero_instructor <- nonzero_entry[as.integer(names(nonzero_entry))%in%instructor_id]
      ...
      for(j in 1:sum(classDay)) 
      {
        ...
        {
          DailyNetSparse[[ seq_len(7)[classDay][j] ]][nonzero_day,nonzero_day] <- DailyNetSparse[[ seq_len(7)[classDay][j] ]][nonzero_day,nonzero_day] + 1 - diag(length(nonzero_day))
        }
        if(num_check>k_reg )
        {
          DailyNetSparse[[ seq_len(7)[classDay][j] ]][nonzero_day,nonzero_day] <- DailyNetSparse[[ seq_len(7)[classDay][j] ]][nonzero_day,nonzero_day] + get.adjacency(sample_k_regular(num_check,k_reg))
        }
      }
    }
...
getgrapgml(DailyNet, full_info_id, output_path, NetName = "ClassNetPlatoon")
